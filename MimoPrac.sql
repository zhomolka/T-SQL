...
/****** Object:  UserDefinedFunction [dbo].[MimoPrac]    Script Date: 3. 1. 2020 11:12:20 ******/
SET ANSI_NULLS ON
GO
	...
	IF @Redirector='9533' -- Call Centrum Brno
	  BEGIN
        ...
        ELSE
		  BEGIN
		     -- Provoz všedního dne
			 IF @CallTime >= convert(datetime, '2019.08.01')
			   BEGIN
				 IF DATETIMEFROMPARTS(year(@CallTime), month(@CallTime), day(@CallTime), 8, 0, 0, 0 )>@CallTime -- do 3.1.2020 tu bylo 7
				   RETURN 1
				 IF DATETIMEFROMPARTS(year(@CallTime), month(@CallTime), day(@CallTime), 20, 0, 0, 0 )<@CallTime -- do 3.1.2020 tu bylo 21
				   RETURN 1
			   END
             ELSE -- Před 2019.08.01
			   BEGIN
        ...
        ELSE
		  BEGIN
		     IF DATEPART(weekday,@CallTime)=7 -- Sobota
			  BEGIN
			     IF @CallTime >= convert(datetime, '2019.08.01')
				   RETURN 1  -- Sobota zařazena mezi svátky
                 ELSE
				   BEGIN
