...
DECLARE @Retezec AS NVARCHAR(100)='%Predistributed%'
...
SELECT TOP 1000 [DataQueryId]
  ...
  FROM .[dbo].[IvrStep] IVS
   LEFT JOIN .[dbo].[IvrScript] IVR ON IVR.IvrScriptId=IVS.IvrScriptId
   WHERE (Targets LIKE @Retezec OR FileName LIKE @Retezec) AND IVS.Deleted=0 AND IVR.Deleted=0
