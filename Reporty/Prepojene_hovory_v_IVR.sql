...
SELECT DISTINCT TOP 100000
	  ...
	  ,IVRST2.Action
	  ,IVRST2.Numbers AS Prepojeno_Na
    FROM [iCC].[dbo].[InboundCall] IC
     --LEFT JOIN [iCC].[dbo].[Workplace] WP ON IC.WorkplaceId=WP.WorkplaceId
	 LEFT JOIN [iCC].[dbo].[CallEvent] CAE ON CAE.InboundCallId=IC.InboundCallId --AND CAE.EventType
	 LEFT JOIN IvrStep AS IVRST2 ON IVRST2.IvrStepId=CAE.ReferenceId
	 --LEFT JOIN IvrStep AS IVRST ON IVRST.IvrStepId=CAE.ReferenceId --AND IVRST.ResultData='TransferedRelease'

  WHERE 1=1
   --AND InboundCallId IN ('e5847637-b18e-e611-80bd-001e67feed3f','31C22EAC-F28A-E611-80F2-F8BC1253A1A4')
    AND PilotTime>@from
	AND IVRST2.Action='Transfer'
	--AND CAE.ResultData='TransferedRelease'
	AND EXISTS(SELECT TOP 1 1 FROM CallEvent CAE2 WHERE CAE2.ResultData='TransferedRelease' AND CAE2.inboundCallid=IC.inboundCallid)
  ORDER BY Numbers,PilotTime