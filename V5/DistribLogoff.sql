...
(SELECT TOP 1 WorkplaceId FROM CallEvent CE WITH(NOLOCK) WHERE CE.TimeUTC>DATEADD(ss,-40,ISNULL(AE.TimeUTC,@Now)) AND 
      CE.TimeUTC<AE.TimeUTC AND CE.EventType=68/*'Distributing'*/ AND AE.Agentid=CE.Agentid) AS WorkplaceId


,AE.TimeUTC, 'Agent' as Kind, AE.EventType AS EventAgent,
