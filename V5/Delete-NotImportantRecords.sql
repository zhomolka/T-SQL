DECLARE @from AS datetime=convert(datetime, '2023.05.11')
DECLARE @to AS datetime=convert(datetime, '2018.09.26 12:30')
DECLARE @cfrom AS NVARCHAR(100)='convert(datetime, '''+CONVERT ( nvarchar(10), @from , 102 )+''')'
DECLARE @cto AS NVARCHAR(100)='convert(datetime, '''+CONVERT ( nvarchar(19), @To , 120 )+''')'
DECLARE @Condition AS NVARCHAR(300)=''
DECLARE @CommonDel AS bit=1
DECLARE @ProjectId AS VARCHAR(36)='B2A76908-1A64-4A14-BA31-F9AE94DA00DE' -- YVES Rocher CZ
DECLARE @OutboundCallId AS VARCHAR(36)='680d9625-ed4a-e911-841f-000c29ebd3f9'
DECLARE @LastTime AS bit=0
DECLARE @Hour AS Integer=(SELECT DATEPART(HOUR,GETDATE()))

DECLARE @RecCount AS Integer=2--120000--2:48min--200000 --5:33-300000 --je asi moc -- count of records in one cycle
DECLARE @Count AS Integer=250--000 -- Count of cycles


IF @LastTime=1
 BEGIN
    SET @from=DATEADD(DAY,-2,GETDATE())
    SET @to=GETDATE()
  END


--select top 1 MessageId AS Id into .dbo.FSC_IDs from .dbo.Message 

  WHILE @Count>0 --AND (@Hour<7 OR @Hour>16)
    BEGIN
	SET @RecCount=IIF(@Hour<7 OR @Hour>17,10000,80)
   PRINT ('Maži tabulku IDs:')
   DELETE FROM .dbo.FSC_IDs

    PRINT ('Vkládám nová ID:')
 INSERT INTO .dbo.FSC_IDs (Id)
 select top (@RecCount) MessageId from .dbo.Message 
 WHERE 1=1
  AND TimeUtc<@from 
  AND MessageType<>101 --'TmpEmail' 
 -- AND MessageResult<>0--'Active'
 --AND ( SpamLevel>=1
 --  OR Direction='O' AND AgentId IS NULL AND SubjectField LIKE '%automatická odpovìï')
IF 1=2 -- Jen informuj
  BEGIN
	 SELECT * FROM .dbo.FSC_IDs
	 RETURN
  END
/* */
    PRINT ('Maži Attachment:')
	--DELETE FROM iCC.dbo.Attachment WHERE MessageId IN (SELECT * FROM ##TEMP)
	DELETE TOP (@RecCount*10) ATC FROM .dbo.Attachment ATC
     INNER JOIN .dbo.FSC_IDs ID ON ATC.MessageId=ID.Id


	    PRINT ('Maži MessageEvent:')
	DELETE TOP (@RecCount*20) ME FROM .dbo.MessageEvent ME
     INNER JOIN .dbo.FSC_IDs ID ON ME.MessageId=ID.Id

	    PRINT ('Maži MessageId FROM ScenarioResult:')
	update TOP (@RecCount*3) SCR 
	set MessageId = NULL 
	FROM .dbo.ScenarioResult SCR
     INNER JOIN .dbo.FSC_IDs ID ON SCR.MessageId=ID.Id

	   PRINT ('Maži RelatedMessageId FROM Message:')
	update TOP (@RecCount*3) ME 
	set RelatedMessageId = NULL 
	FROM .dbo.Message ME
     INNER JOIN .dbo.FSC_IDs ID ON ME.RelatedMessageId=ID.Id

	    PRINT ('Maži Message:')
	DELETE TOP (@RecCount) M FROM .dbo.Message M
     INNER JOIN .dbo.FSC_IDs ID ON M.MessageId=ID.Id
	 SET @Count=@Count-1
	 PRINT ('============================================================')
	 WAITFOR DELAY '00:00:10'
	 SET @Hour =(SELECT DATEPART(HOUR,GETDATE()))
   END


/**/