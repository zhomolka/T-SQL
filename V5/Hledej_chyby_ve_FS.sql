...
/* Definice kontroly               TableName   ColumnName */
insert into @ControlTable values
								...
								('Dataquery','QuerySortExpression')

								
SET @RowsCount = (SELECT COUNT(*) FROM @ControlTable)
DECLARE @ValCounter AS Integer = 0 
/* Definice testovaných hodnot    TestValue    */
DECLARE @ValueTable TABLE (TestValue NVARCHAR(50))
insert into @ValueTable values('FSAdmin'),('iCC'),('FS_Custom'),('ReactClient'),('TimeLocal')
...
SELECT DISTINCT
 ...
 WHERE m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value1+'%' COLLATE DATABASE_DEFAULT OR
 	   m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value1+'.%' COLLATE DATABASE_DEFAULT OR
       m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value1+']%' COLLATE DATABASE_DEFAULT 
...
SELECT DISTINCT
  ...
  FROM sys.sql_modules m
       ...
       sys.objects o
         ON m.object_id = o.object_id
 WHERE m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value2+'%' COLLATE DATABASE_DEFAULT 
 ...
 SELECT DISTINCT
  ...
  FROM sys.sql_modules m
       ...
       sys.objects o
         ON m.object_id = o.object_id
 WHERE m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value3+'%' COLLATE DATABASE_DEFAULT 
 ...
 SELECT DISTINCT
  ...
  FROM sys.sql_modules m
       ...
       sys.objects o
         ON m.object_id = o.object_id
 WHERE m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value4+'%' COLLATE DATABASE_DEFAULT 
 ...
 SELECT DISTINCT
  ...
  FROM sys.sql_modules m
       ...
       sys.objects o
         ON m.object_id = o.object_id
 WHERE m.definition COLLATE DATABASE_DEFAULT Like '%'+@Value5+'%' COLLATE DATABASE_DEFAULT 
