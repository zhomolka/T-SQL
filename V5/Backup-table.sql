...
--USE Frontstage
DECLARE @from AS datetime=convert(datetime, '2024.09.05')
...
DECLARE @MyId AS NVARCHAR(36)='a2024e60-1b9b-ee11-83c7-005056013adb'
...
-- do 26.9.2018 14:30
 ...
 --SELECT @cto
DECLARE @MyTable AS NVARCHAR(40)='Message' --'OutboundCall' --'InboundCall'--'ScenarioResultValue'--'Chat'-- 'Attachment' --'ChangeRequest' --'IVRStep' --'IVRScript' --'Holiday' --'Issue'--'WaitPostcondition' --'Issue' --'OutboundCall' -- --'DataQuery'----  
...
IF @MyTable='Message'
  BEGIN
   -- POZOR - Na Azure je potøeba mazat pøes nastavení GDPR !!!!!!! je to v blobech.
   SET @Condition =' WHERE 
      Gatewayid=''f8e38ef1-e6cb-4514-9764-580707b02a04''
  AND SubjectField LIKE ''Yenkee - web dotaz%''
  AND Direction=''I''
  AND TimeUtC> '+@cfrom
...
IF @MyTable='InboundCall'
  BEGIN
   ...
   SET @Condition =' WHERE InboundCallId = ''+@MyId+'''
...
IF @MyTable='OutboundCall'
  BEGIN
   SET @Condition =' WHERE ProjectId = '''+@ProjectId+''' AND Timeutc<'+@cto
   SET @Condition =' WHERE OutboundCallId = '''+@MyId+''''
    -- SELECT 'select OutboundCallId into ##TEMP from .dbo.OutboundCall'+@Condition
 ...
 BEGIN
	...
	IF @MyTable='Message'
	  BEGIN
		...
		DELETE FROM .dbo.MessageEvent WHERE MessageId IN (SELECT * FROM ##TEMP)
            PRINT ('Maži Queue:')
		DELETE  FROM [Queue]  WHERE CommId IN (SELECT * FROM ##TEMP)
	...
	IF @MyTable='Issue'
	  BEGIN  	
		...
		WHERE IssueId IN (SELECT * FROM ##TEMP))
                UPDATE Issue SET FormdataId=NULL WHERE IssueId IN (SELECT * FROM ##TEMP)
